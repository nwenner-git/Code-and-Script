...
if ! [ -d "/Applications/Safari.app" ]; then
    echo "Safari is not installed on your system."
    exit 1
fi

# Check for updates
echo "Checking for Safari updates..."
softwareupdate --list --include-config-data | grep "Safari"
if [ $? -ne 0 ]; then
    echo "Failed to check for Safari updates."
    exit 1
fi

# Download and install updates
echo "Downloading and installing Safari updates..."
softwareupdate --install "Safari"
if [ $? -ne 0 ]; then
    echo "Failed to download and install Safari updates."
    exit 1
fi

echo "Safari has been updated successfully."
